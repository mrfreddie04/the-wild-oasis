...
function Open({ opensWindowName, children }) {
  ...
  //return <button onClick={()=>open(opens)}>{icon}</button>;
  // 1-element to clone, 2- props added to the cloned element
...
function Window({ name, children }) {
  ...
  //check if this is the currently opened window
  if (openName !== name) {
    return null;
  }
  ...
  //     <StyledModal onClick={(e) => e.stopPropagation()} ref={modal}>
  //       <Button onClick={close}>
  //         <HiXMark />
  //       </Button>
  //       <div>{cloneElement(children, { onCloseModal: () => close() })}</div>
